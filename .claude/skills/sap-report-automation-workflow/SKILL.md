...
代理在**同一任务内**按阶段 **0→1→1.5→2→2.5→3→3.5→3.6→4→5→5.5 顺序自动跑完**，仅在缺关键输入、SAP 不可达、或达重试上限时停顿。

**硬门禁**（除非用户明确说跳过）：
0. `test_rfc.js` 未通过 → 禁止进入阶段 1。代理必须指导用户完成 NW RFC SDK 安装和 `.env` 配置。
1. 无 `functional-spec-ai.md` → 禁止写 ABAP、禁止部署。
2. 无 metadata JSON 落盘 → 禁止进入阶段 4。
3. 无 `tech-design.md`（含字段契约）→ 禁止进入阶段 4。
...
必须包含：**字段契约**（FS 列/表.字段/metadata 溯源）、表清单、关联路径、取数逻辑、选择屏↔数据库映射、ALV 布局、性能设计（内表类型/嵌套 LOOP 替代/WHERE 排列/数据量预估）。

**S3 门禁**：`tech-design.md` 含字段契约；`fs-coverage.md` 覆盖全部输出列；`template-mapping.md` 列出新旧 INCLUDE 对应；`deployment-config.md` 已生成。
...
**S4 门禁**：ASSIGN COMPONENT 字段名对齐 + 6 轮自检通过 + 源码结构与 template-mapping.md 一致。
...
```bash
# 在数据系统上取真实数据
node scripts/rfc_client.js --env=.env.data --rows=1 \
  --sql "SELECT * FROM <主表> WHERE <条件>" --table <主表>
```

手工计算预期输出，对比程序逻辑。记录到 `smoke-test.md`。
...
**评判铁律**：预期值必须来自源表手工计算；金额为 0 ≠ Bug；子范围 = 全范围 ≠ Bug（先查源表分布）。

### 5.5.3 ALV 列核对

静态分析源码，确认列数与 `fs-coverage.md` 一致。

**S5.5 门禁**：三步不可跳过（源表采样 → 手工预期 → verify_report.js 多组比对）；期初/本期/累计/期末四组列各自至少一条非零；`smoke-test.md` 已生成。
...
| 5.5 | 数据采样 | `node scripts/rfc_client.js --env=.env.data --rows=5 --sql "SELECT * FROM <表>" --table <表>` |
...
| 新增/替换透明表 | S2 | 2, 2.5, 3, 3.5, 4, 5, 5.5 |
...
- [ ] S4: ASSIGN COMPONENT 字段名对齐 + sy-subrc 检查 + 6 轮自检通过
- [ ] S4: 源码结构与 template-mapping.md 一致
- [ ] S5: deploy_rfc.js 成功，激活无错误
- [ ] S5.5: 源表采样 → 手工预期 → verify_report.js 多组比对 → 四组列全检查
- [ ] S5.5: smoke-test.md 已生成
