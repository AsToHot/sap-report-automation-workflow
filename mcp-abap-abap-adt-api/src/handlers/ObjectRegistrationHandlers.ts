...
export class ObjectRegistrationHandlers extends BaseHandler {
  ...
  async handleCreateObject(args: any): Promise<any> {
    const startTime = performance.now();
    try {
      // abap-adt-api's createObject works correctly for all types including PROG/I.
      // For PROG/I, the object is created at /sap/bc/adt/programs/includes/{name}
      // (not under the parent program). The objectPath returned by abap-adt-api
      // is correct for creation but callers need the right URI for lock/source.
      ...
      const objectUrl = args.objtype === 'PROG/I'
        ? `/sap/bc/adt/programs/includes/${args.name.toLowerCase()}`
        : undefined;
