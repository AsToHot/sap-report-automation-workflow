...
import { ProxyHttpClient } from './lib/ProxyHttpClient.js';
...
export class AbapAdtServer extends Server {
  ...
  private objectRegistrationHandlers: ObjectRegistrationHandlers;
    ...
    const missingVars = ['SAP_URL'].filter(v => !process.env[v]);
    if (missingVars.length > 0) {
      throw new Error(`Missing required environment variables: ${missingVars.join(', ')}`);
    }

    // Use ProxyHttpClient to bypass axios, CSRF, cookies, and Basic Auth.
    // The local RFC proxy (rfc-proxy-server.js) handles authentication at
    // the RFC layer, so HTTP-level session management is unnecessary.
    const httpClient = new ProxyHttpClient(process.env.SAP_URL as string);
    this.adtClient = new ADTClient(httpClient, 'dummy', 'dummy', '', '', {});
  ...
  private setupToolHandlers() {
    this.setRequestHandler(ListToolsRequestSchema, async () => {
      return {
        tools: [
          // Auth tools removed: RFC proxy handles authentication at the transport layer.
