...
async function run() {
  try {
    ...
    for (const incName of ["zsap_fi086at01", "zsap_fi086asel", "zsap_fi086af01"]) {
      // Try to unlock without lockHandle (force release user locks)
      try {
        const resp = await adtRequest("POST", `/sap/bc/adt/programs/includes/${incName}?_action=UNLOCK`, {
          headers: { Accept: "application/vnd.sap.as+xml" },
        });
        console.log(`[OK] Unlock ${incName}: ${resp.statusCode} ${resp.statusText}`);
      } catch (e) {
        console.log(`[INFO] Unlock ${incName}: ${e.statusCode || e.message}`);
      }
    }
